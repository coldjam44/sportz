...
class CreatestoreController extends Controller
{
    ...
    public function getStoreSectionsAndProducts(Request $request)
    {
        ...
        if (!$storeId) {
            $query = \App\Models\addproduct::with(['section.stores'])
                ->where('is_hidden', 0) // تجاهل المنتجات المخفية
                ->whereHas('section.stores', function ($q) {
                    $q->where('is_hidden', 0); // تجاهل المنتجات التي تنتمي لمتاجر مخفية
                });
        ...
        $transformed = $products->getCollection()->map(function ($product) use ($cartItems, $favouriteProductIds) {
            ...
            return [
                ...
                'store' => $store ? [
                    'id' => $store->id,               // هنا أضفت id الخاص بالمتجر
                    'name' => $store->name,
                    'location' => $store->location,
    ...
    public function getFavourites(Request $request)
    {
        ...
        $transformed = $favourites->getCollection()->map(function ($fav) use ($cartItems, $favouriteProductIds, $now) {
            $product = $fav->product;
